...
<body class="antialiased bg-stone-50 text-stone-900 dark:bg-[#0c0a09] dark:text-[#d6d3d1] selection:bg-amber-700 selection:text-white pb-32 md:pb-12 pt-24 md:pt-32 transition-colors duration-300">
    ...
    <main class="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        <header class="mb-16 md:mb-24 mt-8 md:mt-12">
            ...
            <h1 class="font-heading text-4xl md:text-5xl lg:text-6xl text-stone-900 dark:text-stone-100 tracking-tight mb-4">Commission a Unique Piece</h1>
            <p class="text-lg md:text-xl text-stone-600 dark:text-stone-400 max-w-2xl leading-relaxed">Share your vision and request a quote from <strong>{{ $artisanUser->name }}</strong>.</p>
        </header>
        ...
        <form action="{{ route('booking.store', $artisanUser->id) }}" method="POST" enctype="multipart/form-data" class="space-y-16 md:space-y-24">
            ...
            <section class="glass-card rounded-xl p-8 md:p-12 shadow-sm relative z-10">
                <h2 class="font-heading text-2xl text-stone-900 dark:text-stone-100 mb-8 border-b border-stone-200 dark:border-stone-800 pb-4">Project Vision</h2>
                <div class="space-y-8">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-2" for="title">Project Title</label>
                        <input class="w-full bg-stone-50 dark:bg-stone-900 border-none border-b border-stone-300 dark:border-stone-700 rounded-none px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-amber-600 transition-all text-stone-900 dark:text-stone-100 placeholder:text-stone-400" id="title" name="title" placeholder="e.g., Walnut Dining Table for 12" type="text" required value="{{ old('title') }}"/>
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-2" for="description">Detailed Description</label>
                        <textarea class="w-full bg-stone-50 dark:bg-stone-900 border-none border-b border-stone-300 dark:border-stone-700 rounded-none px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-amber-600 transition-all text-stone-900 dark:text-stone-100 placeholder:text-stone-400 resize-y" id="description" name="description" placeholder="Describe the materials, style, dimensions, and any specific requirements..." rows="5" required>{{ old('description') }}</textarea>
                    </div>
                </div>
            </section>

            <!-- Section 2: Logistics -->
            <section class="bg-stone-100 dark:bg-stone-900/50 rounded-xl p-8 md:p-12 border border-stone-200 dark:border-stone-800">
                <h2 class="font-heading text-2xl text-stone-900 dark:text-stone-100 mb-8 border-b border-stone-200 dark:border-stone-800 pb-4">Logistics & Details</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-2" for="budget_range">Proposed Budget / Price</label>
                        <input class="w-full bg-white dark:bg-stone-900 border-none border-b border-stone-300 dark:border-stone-700 rounded-none px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-amber-600 transition-all text-stone-900 dark:text-stone-100 placeholder:text-stone-400" id="budget_range" name="budget_range" placeholder="e.g. $5,000" type="text" value="{{ old('budget_range') }}"/>
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-2" for="scheduled_date">Desired Completion Date</label>
                        <input class="w-full bg-white dark:bg-stone-900 border-none border-b border-stone-300 dark:border-stone-700 rounded-none px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-amber-600 transition-all text-stone-900 dark:text-stone-100" id="scheduled_date" name="scheduled_date" type="date" required value="{{ old('scheduled_date') }}"/>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-2" for="location">Delivery Location (City, Country)</label>
                        <input class="w-full bg-white dark:bg-stone-900 border-none border-b border-stone-300 dark:border-stone-700 rounded-none px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-amber-600 transition-all text-stone-900 dark:text-stone-100 placeholder:text-stone-400" id="location" name="location" placeholder="e.g., London, UK" type="text" value="{{ old('location') }}"/>
                    </div>
                </div>
            </section>
            ...
            <section class="glass-card rounded-xl p-8 md:p-12 shadow-sm">
                <div class="flex justify-between items-end mb-8 border-b border-stone-200 dark:border-stone-800 pb-4">
                    <h2 class="font-heading text-2xl text-stone-900 dark:text-stone-100">Inspirations & References</h2>
            ...
            <div class="pt-8 flex justify-end">
                <button type="submit" class="bg-amber-700 hover:bg-amber-800 text-stone-50 text-sm font-bold tracking-widest uppercase px-8 py-4 rounded transition-colors shadow-md hover:shadow-lg dark:shadow-[0_0_15px_rgba(217,119,6,0.2)] dark:hover:shadow-[0_0_20px_rgba(217,119,6,0.4)] flex items-center gap-3">
                    Submit Inquiry
