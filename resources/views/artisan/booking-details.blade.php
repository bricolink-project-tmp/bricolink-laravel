    ...
    <main class="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        <header class="mb-12 mt-8 md:mt-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            ...
            @if($booking->user->profile_pic)
                <img src="{{ asset('storage/' . $booking->user->profile_pic) }}" alt="Profile" class="w-12 h-12 rounded-full object-cover border border-stone-200">
            @else
                <div class="w-12 h-12 rounded-full bg-stone-100 border border-stone-200 text-sm flex items-center justify-center font-bold text-stone-500">{{ substr($booking->user->name, 0, 2) }}</div>
            @endif
                <div>
                    <div class="text-[10px] text-stone-500 uppercase font-bold tracking-widest mb-0.5">Client</div>
                    <div class="font-bold text-stone-900 text-sm">{{ $booking->user->name }}</div>
                </div>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            ...
            <div class="space-y-6">
                ...
                <div class="glass-card rounded-xl p-6 border-t-4 border-t-amber-500">
                    <h3 class="text-xs font-bold uppercase tracking-widest text-stone-500 mb-4">Response Actions</h3>
                    ...
                    <form action="{{ route('booking.artisan.status', $booking->id) }}" method="POST" class="mb-3">
                        ...
                        <button type="submit" class="w-full bg-amber-700 hover:bg-amber-800 text-white text-sm font-bold uppercase tracking-widest py-3 px-4 rounded transition-colors shadow-sm flex items-center justify-center gap-2">
                    ...
                    <form action="{{ route('booking.artisan.status', $booking->id) }}" method="POST">
                        ...
                        <button type="submit" class="w-full bg-stone-200 hover:bg-stone-300 text-stone-600 text-xs font-bold uppercase tracking-widest py-3 px-4 rounded transition-colors">
                            Decline Request
                        </button>
                    </form>
                </div>
                @elseif($booking->status === 'in_discussion')
                    <div class="glass-card rounded-xl p-6 border-t-4 border-t-amber-500 shadow-sm relative z-20">
                        <h3 class="text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-4">Propose Final Quote</h3>
                        <form action="{{ route('booking.artisan.status', $booking->id) }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-stone-600 mb-1">Final Price ($)</label>
                                <input type="number" name="price" step="0.01" min="0" class="w-full bg-stone-50 border border-stone-200 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-amber-500 transition-colors text-stone-900" required>
                            </div>
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-stone-600 mb-1">Terms & Conditions</label>
                                <textarea name="final_terms" rows="3" class="w-full bg-stone-50 border border-stone-200 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-amber-500 transition-colors text-stone-900" placeholder="Timeline, inclusions, materials..." required></textarea>
                            </div>
                ...
                @elseif($booking->status === 'artisan_approved')
                    <div class="glass-card rounded-xl p-6 border border-amber-200 bg-amber-50/50">
                        ...
                        <p class="text-xs text-stone-600">You proposed a final price of <strong class="text-stone-900">${{ number_format($booking->price, 2) }}</strong>. Waiting for the client to accept and book.</p>
                    </div>
                @elseif($booking->status === 'booked')
                    <div class="glass-card rounded-xl p-6 border-t-4 border-t-emerald-500 bg-emerald-50/30">
                        <h3 class="text-[10px] font-bold uppercase tracking-widest text-emerald-700 mb-2">Job is Active</h3>
                        <p class="text-[11px] text-stone-600 mb-4 leading-relaxed">When you have completed the work and delivered it to the client, mark it as finished here.</p>
                        <form action="{{ route('booking.artisan.status', $booking->id) }}" method="POST">
                            @csrf
                ...
                @elseif($booking->status === 'artisan_completed')
                    <div class="glass-card rounded-xl p-6 border border-emerald-200 bg-emerald-50/50">
                        ...
                        <p class="text-[11px] text-stone-600">You marked this job as completed. Waiting for the client to verify and leave a review.</p>
                    </div>
                @elseif($booking->status === 'completed')
                    <div class="glass-card rounded-xl p-6 border-l-4 border-l-emerald-500 bg-white">
                        <h3 class="text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-4">Final Verdict</h3>
                ...
                @elseif($booking->status === 'rejected_by_client')
                    <div class="glass-card rounded-xl p-6 border border-red-200 bg-red-50/50">
                        ...
                        <p class="text-[11px] text-stone-600 mb-4">The client declined your formal quote of <strong class="text-stone-900">${{ number_format($booking->price, 2) }}</strong>.</p>
                        <form action="{{ route('booking.artisan.status', $booking->id) }}" method="POST">
                            @csrf
                            <input type="hidden" name="status" value="archived">
                            <button type="submit" class="w-full bg-stone-200 hover:bg-stone-300 text-stone-700 font-bold uppercase tracking-widest py-2 rounded text-[10px] transition-colors">Archive</button>
                        </form>
                    </div>
                @endif
            </div>
