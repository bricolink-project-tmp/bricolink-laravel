...
<body class="antialiased bg-stone-50 text-stone-900 dark:bg-[#0c0a09] dark:text-[#d6d3d1] selection:bg-amber-700 selection:text-white pb-20 transition-colors duration-300">
    ...
    <nav class="fixed w-full z-50 glass-card border-b border-stone-200 dark:border-b-0 dark:border-stone-800 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 text-stone-500 hover:text-amber-700 dark:hover:text-amber-500 transition-colors group">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <span class="hidden sm:inline-block text-sm text-stone-500 dark:text-stone-400"><span class="text-stone-900 dark:text-stone-200">{{ explode(' ', $user->name)[0] }}</span>'s Settings</span>
                </div>
            </div>
        </div>
    </nav>
    ...
    <main class="pt-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-12">
            <h1 class="font-heading text-4xl font-bold text-stone-900 dark:text-stone-100 mb-2">Profile & Preferences</h1>
            <p class="text-stone-500 dark:text-stone-400">Manage how clients discover and engage with you.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="glass-card rounded-lg p-6 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30">
                    <h3 class="text-amber-800 dark:text-amber-500 font-bold uppercase tracking-widest text-xs mb-3">Standing Out</h3>
                    <p class="text-stone-600 dark:text-stone-400 text-sm leading-relaxed mb-4">Clients are 3x more likely to contact artisans who have detailed bios. Mention:</p>
                    <ul class="text-xs text-stone-600 dark:text-stone-400 space-y-2 list-disc pl-4">
                ...
                <div class="glass-card rounded-xl p-8 shadow-sm">
                    <form action="{{ route('artisan.profile.update') }}" method="POST">
                        ...
                        <div class="mb-8 relative">
                            <label class="block text-xs font-bold uppercase tracking-widest text-stone-600 dark:text-stone-400 mb-3">Craft Category</label>
                            <p class="text-xs text-stone-500 mb-3">Select the primary category clients will use to filter their searches for you.</p>
                            <div class="relative">
                                <select name="craft_type" class="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded p-4 text-sm text-stone-800 dark:text-stone-200 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors appearance-none cursor-pointer">
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="bio" rows="8" class="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded p-4 text-sm text-stone-800 dark:text-stone-200 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors leading-relaxed" placeholder="Passionate artisan dedicated to high-quality craftsmanship...">{{ $user->artisan->bio }}</textarea>
                        </div>

                        <div class="flex justify-end pt-4 border-t border-stone-200 dark:border-stone-800">
                            <button type="submit" class="bg-amber-700 hover:bg-amber-800 dark:bg-amber-700 dark:hover:bg-amber-600 text-stone-50 text-xs font-bold uppercase tracking-widest py-4 px-8 rounded transition-all shadow-md hover:shadow-lg dark:shadow-[0_0_15px_rgba(217,119,6,0.2)] dark:hover:shadow-[0_0_20px_rgba(217,119,6,0.4)]">
                                Save Settings
                            </button>
                        </div>
                    </form>
